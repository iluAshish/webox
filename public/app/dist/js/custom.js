$(document).ready(function () {
    ...
    if ($('.placeholder-cls').val() == '' || $('.placeholder-cls').val() == null) {
        $('.placeholder-cls').val('alt="Container Solution"');
    }
    ...
    $(".nameField").keypress(function (e) {
        var key = e.keyCode;
        if (!((key == 95) || (key == 32) || (key >= 65 && key <= 90) || (key >= 97 && key <= 122))) {
            e.preventDefault();
        }
    });
    ...
    $('.common_sort_order, .sort_order').keypress(function (e) {
        var key = e.keyCode;
        if (!(key >= 48 && key <= 57)) {
            e.preventDefault();
        }
    ...
    //     if($(".titleField").value.length < 3 && $(".titleField").value.length > 60){
    //         $('#title_error').html('').css({'color':'#FF0000','font-size':'14px'});
    //     }
    // });
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.status == false) {
                    ...
                    swal('Success !', 'Sort order has been updated successfully', 'success');
                }
            }
        })
    });
    ...
    $(document).on('click', '#delete_newsletter_btn', function () {
        var _token = token;
        var id = $('#ids').val();
        swal({
            title: "Are you sure?",
            ...
            if (isConfirm) {
                $.ajax({
                    type: 'POST',
                    dataType: 'json',
                    url: base_url + '/newsletter/delete_multi_newsletter/',
                    data: {id: id, _token: _token},
                    success: function (data) {
                        if (data.status == false) {
                            swal('Error !', data.message, 'error');
    ...
    $(document).on('click', '#replay_to_enquiry', function (e) {
        e.preventDefault();
        var url = $('#url').val();
        $('#replay_to_enquiry').val('Please Wait..!');
        $.ajax({
            type: 'POST',
            dataType: 'json',
            ...
            success: function (response) {
                $('#replay_to_enquiry').val('Update Replay');
                if (response.status == true) {
                    ...
                    if(response.message == 'Already sent reply'){
                        swal('Message !', response.message, 'info');
                    }else{
                        swal('Error !', response.message, 'error');
                    }
    ...
    $(document).on('change', '.status_check', function (event) {
        ...
        var _token = token;
        $.ajax({
            type: 'POST', dataType: 'json', url: base_url + url, data: {
                'state': state,
                table: table,
    ...
    $('.headingSubmit').on('click', function (e) {
        e.preventDefault();
        ...
        // console.log($('#homeHeadingForm').serialize());
        ...
        if($('.kv-error-close').length > 0){
            required.push($('.kv-error-close').val());
        }
        if (required.length == 0) {
            if($('input[name=formSubmission]').length > 0){
                $('#'+formId).submit();
            }
            else{
                $('.loadingImg').show();
    ...
    //         state = state;
    //         $.ajax({
    //             type: 'POST', url: base_url + '/status-change', data: {
    //                 state: state, table: table, primary_key: primary_key, _token: _token, field: field
    //             }, success: function (response) {
    //                 if (response == "1") {
    ...
    // if ($('.bool_status').length) {
    //     $(document).on('change', '.bool_status', function (event, state) {
    //         var id = $(this).data('id');
    //         var url = $(this).data('url');
    //         var _token = token;
    //         state = state;
    ...
    if ($('.is_featured').length) {
        $(document).on('switchChange.bootstrapSwitch', '.is_featured', function (event, state) {
            var id = $(this).data('id');
            var url = $(this).data('url');
            var _token = token;
            state = state;
        ...
        $('.required').each(function (k, v) {
            ...
            if ($(this).hasClass('tinyeditor')) {
                ...
                if (field_name == 'email_id') {
                    ...
                    if (!regex.test($(v).val())) {
                        required.push($('#' + id).val());
                        $('#' + id + '_error').html('Please enter a valid email address').css({
                            'color': '#FF0000', 'font-size': '14px'
                        });
                    } else {
                        $('#' + id + '_error').html('');
                ...
                if (field_name == 'name') {
                    var regex = /^[a-zA-Z]+(\s[a-zA-Z_]*){0,2}$/;
                    if (!regex.test($(v).val())) {
                        required.push($('#' + id).val());
                        $('#' + id + '_error').html('Please enter a valid name').css({
                            'color': '#FF0000', 'font-size': '14px'
                        });
                    } else {
                        $('#' + id + '_error').html('');
                ...
                //     var regex = /(^\d{7,15}$)|(^\d{5}-\d{4}$)/;
                //     if (!regex.test($(v).val())) {
                //         required.push($('#'+id).val());
                //         $('#'+id+'_error').html('Please enter a valid phone number').css({'color':'#FF0000','font-size':'14px'});
                //     }else{
                //         $('#'+id+'_error').html('');
                //     }
                // }
    ...
    //
    // $(document).on('click', '.login_form_submit_btn', function (e) {
    //     e.preventDefault();
    ...
    //     var errors = false;
    //
    //     $('form input, form textarea, form select').removeClass('is-invalid is-valid');
    //     $('span.error').remove();
    //     var  required_val = ' .required';
    //     $("#" + form_id + required_val).each(function (k, v) {
    //         var field_name = $(v).attr('name');
    //         if (!$(v).val().length) {
    //             errors = true;
    //             var error = 'The <strong>' + field_name.replace(/_/g, ' ') + '</strong> field is required.';
    ...
    //     });
    //     if (!errors) {
    //         $('#'+form_id+' .form_submit_btn').html('Please Wait..');
    //         $.ajax({
    //             type: 'POST',
    //             dataType: 'json',
    //             cache: false,
    ...
    //                 $('#'+form_id+' .form_submit_btn').html(buttonText);
    ...
    $(document).on('click', '.add_detail_row', function () {
        ...
        $.ajax({
            type: 'POST',
            data: {unique_id: unique_id, _token: _token, program_id: program_id},
            url: base_url + '/program/details/add_row',
            success: function (response) {
                $('.add_detail_row').hide();
                $(response).hide().insertAfter("#append_result_" + unique_id).fadeIn(500);
                tinymce.EditorManager.execCommand('mceAddEditor',true, 'description_'+plus_unique);
    ...
    $(document).on('click', '.remove_detail_row', function () {
        ...
        if (data_key == 0) {
            $(this).closest('.form-row').fadeOut(300, function () {
                $(this).remove();
                tinymce.EditorManager.execCommand('mceRemoveEditor',true, 'description_'+primary_key);
                $('.add_detail_row').hide();
                $('.add_detail_row:last').show();
                tinymce.remove(primary_key);
                $('.submitBtn').val('Submit').attr('disabled', false);
            });
        } else {
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        type: 'POST',
                        dataType: 'json',
                        url: base_url + '/program/details/remove_row',
                        ...
                        success: function (data) {
                            if (data.status == false) {
                                ...
                                swal({title: "Success", text: "Entry has been deleted!", type: "success"}, function () {
                                    ...
                                    $('.add_specification_row:last').show();
                                    $('.submitBtn').val('Submit').attr('disabled', false);
    ...
    /******* Team specifications clone menu ends *********/
    ...
    if ($('.select2').length > 0) {
        $('.select2').select2({
            theme: 'bootstrap4', placeholder: 'select Item'
        });
    }
    ...
    document.getElementById('changePassword').onchange = function() {
        const refresh_code_box = document.getElementById('refresh_code_box');
        var input_box = document.getElementById('login_password');

        $checkbox = document.getElementById("changePassword").checked;

        if($checkbox == true) {
            refresh_code_box.style.display = 'flex';
            input_box.disabled = false;
            passwordGenerate();
        } else {
            refresh_code_box.style.display = 'none';
